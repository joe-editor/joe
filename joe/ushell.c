...
int urun(W *w, int k)
{
	...
	s = ask(w, joe_gettext(_("Program to run: ")), &runhist, "Run",
	        cmplt_command, locale_map, PWFLAG_COMMAND, 0, NULL);
	...
	if (s) {
		...
		a = vaadd(a, cmd);
		a = vaadd(a, s);
		return cstart(bw, "/bin/sh", a, NULL, 0, 0, NULL, SHELL_TYPE_RAW);
	} else {
		return -1;
	}
}
...
int ubuild(W *w, int k)
{
	...
	if (s) {
		...
		a = vaadd(a, t);
		return cstart(bw, "/bin/sh", a, NULL, 1, 0, NULL, SHELL_TYPE_DUMB);
	} else {
		return -1;
	}
}
